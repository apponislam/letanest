...
const HostVerifyPage = () => {
    ...
    return (
        <div className="container mx-auto">
            ...
            <div className="w-full">
                <div className="bg-[#2D3546] border border-[#C9A94D] p-4 md:p-6 text-[#C9A94D] rounded-[20px] space-y-6">
                    {/* Header Section */}
                    <div className="flex justify-between items-center gap-4 flex-col md:flex-row">
                        <h2 className="text-[#C9A94D] text-xl md:text-2xl font-bold">Review ({verifications.length})</h2>

                        <div className="flex items-center gap-3">
                            <span className="text-[#C9A94D] text-sm md:text-base">Filter By Status:</span>
                            <Select value={statusFilter} onValueChange={setStatusFilter}>
                                <SelectTrigger className="bg-[#434D64] border border-[#C9A94D] text-[#C9A94D] rounded-[10px] w-40 md:w-48">
                                    <SelectValue placeholder="All Status" />
                                </SelectTrigger>
                                <SelectContent className="bg-[#2D3546] border border-[#C9A94D] text-white rounded-[10px]">
                                    <SelectItem value="all">All Status</SelectItem>
                    ...
                    <div className="space-y-4">
                        {verifications.length === 0 ? (
                            <div className="text-center py-12 text-[#C9A94D] text-lg">No verifications found</div>
                        ) : (
                            verifications.map((verification: any) => (
                                <div key={verification._id} className="p-4 rounded-[12px] border border-[#C9A94D] bg-[#1E2432]">
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        {/* Left Section - Verification Info */}
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 flex-1">
                                            <div className="text-center md:text-left">
                                                <p className="font-bold text-sm text-gray-300 mb-1">Date</p>
                                                <span className="text-white">{new Date(verification.submittedAt).toLocaleDateString()}</span>
                                            </div>
                                            <div className="text-center md:text-left">
                                                <p className="font-bold text-sm text-gray-300 mb-1">Name</p>
                                                <span className="text-white">
                                                    {verification.firstName} {verification.lastName}
                                                </span>
                                            </div>
                                            <div className="text-center md:text-left">
                                                <p className="font-bold text-sm text-gray-300 mb-1">Email</p>
                                                <span className="text-white text-sm break-all">{verification.userId?.email}</span>
                                            </div>
                                            <div className="text-center md:text-left">
                                                <p className="font-bold text-sm text-gray-300 mb-1">Status</p>
                                                <span className={`inline-block px-3 py-1 text-white rounded-full text-xs font-medium ${verification.status === "approved" ? "bg-green-500" : verification.status === "rejected" ? "bg-red-500" : verification.status === "pending" ? "bg-yellow-500" : "bg-blue-500"}`}>{verification.status?.charAt(0).toUpperCase() + verification.status?.slice(1)}</span>
                                            </div>
                                        </div>

                                        {/* Right Section - Actions */}
                                        <div className="flex items-center gap-3 justify-center md:justify-end">
                                            <button className="bg-[#C9A94D] text-white rounded-[8px] font-bold text-sm py-2 px-4 hover:bg-[#B89A45] transition-colors min-w-[120px]" onClick={() => openHostDetails(verification)}>
                                                View Details
                                            </button>
