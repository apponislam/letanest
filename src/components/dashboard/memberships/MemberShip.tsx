...
import { useAppSelector } from "@/redux/hooks";
import { currentUser } from "@/redux/features/auth/authSlice";
import { useGetAllSubscriptionsAdminQuery, useGetSubscriptionsByTypeForAdminQuery, useToggleSubscriptionStatusMutation, useDeleteSubscriptionMutation } from "@/redux/features/subscription/subscriptionApi";
import { toast } from "sonner";
import { ISubscription } from "@/redux/features/subscription/subscriptionApi";
import { Button } from "@/components/ui/button";
...
const MemberShip = () => {
    ...
    const [activeTab, setActiveTab] = useState<"GUEST" | "HOST">("GUEST");
    const [expandedPlan, setExpandedPlan] = useState<string | null>(null);
    const [deletingId, setDeletingId] = useState<string | null>(null); // ADD STATE FOR LOADING
    ...
    // Mutations
    const [toggleStatus] = useToggleSubscriptionStatusMutation();
    const [deleteSubscription] = useDeleteSubscriptionMutation(); // ADD DELETE MUTATION
    ...
    const handleDelete = async (id: string, name: string) => {
        ...
        try {
            ...
            await Swal.fire({
                title: "Deleted!",
                text: `Subscription "${name}" has been deleted successfully.`,
                icon: "success",
    ...
    const renderPlanCard = (plan: ISubscription) => (
        <div key={plan._id} className="bg-[#2D3546] border border-[#2D3546] hover:bg-transparent hover:border-[#af8d28] p-6 rounded-[16px] flex flex-col justify-between">
            ...
            <div>
                <div className="flex justify-between items-start mb-2">
                    <h2 className="text-[18px] font-semibold text-white">{plan.name}</h2>
                    {plan.badge && <span className="bg-[#C9A94D] text-white text-xs px-2 py-1 rounded-full w-32 text-center">{plan.badge}</span>}
                </div>
            ...
            <div className="flex mt-4 flex-col gap-3">
                ...
                {/* DELETE BUTTON */}
                <Button onClick={() => handleDelete(plan._id!, plan.name)} disabled={deletingId === plan._id} variant="destructive" className="flex items-center justify-center gap-2 w-full">
                    {deletingId === plan._id ? (
                        <>
