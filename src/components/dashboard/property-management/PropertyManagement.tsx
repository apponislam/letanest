...
import { useChangePropertyStatusMutation, useGetAllPropertiesForAdminQuery, useGetPublishedPropertiesForAdminQuery, useTogglePropertyCalendarMutation, useTogglePropertyFeaturedMutation, useTogglePropertyTrendingMutation } from "@/redux/features/property/propertyApi";
                                            ...
                                            <div key={property._id} className="p-4 rounded-[12px] flex justify-between items-center border border-[#C9A94D] flex-col md:flex-row">
                                                <div className="flex items-center gap-4 flex-1 flex-col md:flex-row w-full md:w-auto">
                                                    ...
                                                    <div className="flex-1 w-full">
                                                        <div className="flex  justify-between flex-col md:flex-row w-full md:w-auto gap-2 items-center">
                                                            <div>
                                                    ...
                                                    <div className="flex-1 w-full">
                                                        <div className="flex items-center justify-between flex-col md:flex-row w-full md:w-auto gap-2">
                                                            ...
                                                            <div className="flex items-center flex-col">
                                                                <div className="flex items-center gap-2">
                                                                    <span className={`px-3 py-1 rounded-full text-xs text-white font-medium ${getStatusColor(property.status)}`}>{property.status}</span>
                                                                    <span className="text-xs bg-[#2D3546] px-2 py-1 rounded">#{property.propertyNumber}</span> {property.featured && <span className="px-2 py-1 bg-yellow-600 text-white text-xs rounded">Featured</span>}
                                                                </div>
                                                                <button onClick={() => handleToggleCalendar(property._id, !property.calendarEnabled)} className={`mt-2 px-3 py-1 rounded text-xs font-medium transition-colors ${property.calendarEnabled ? "bg-red-500 hover:bg-red-600 text-white" : "bg-green-500 hover:bg-green-600 text-white"}`}>
                                                                    {property.calendarEnabled ? "Close Calendar" : "Open Calendar"}
                                                                </button>
