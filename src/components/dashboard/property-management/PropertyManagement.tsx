...
const PropertyManagement = () => {
    ...
    console.log(nonPublishedData);

    const { data: publishedData, isLoading: publishedLoading } = useGetPublishedPropertiesForAdminQuery(activeTab === "published" ? filters : undefined);

    console.log(publishedData);

    // Select data based on active tab
    const data = activeTab === "non-published" ? nonPublishedData : publishedData;
    const isLoading = activeTab === "non-published" ? nonPublishedLoading : publishedLoading;
            ...
            <div className="w-full">
                <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
                    ...
                    <TabsList className="flex h-auto overflow-hidden bg-transparent gap-3 mb-5 md:mb-6 w-full flex-col md:flex-row">
                        <TabsTrigger value="non-published" className="p-3 h-auto rounded-[10px] w-full md:w-1/2 text-white data-[state=active]:bg-[#135E9A] data-[state=active]:border-[#C9A94D] data-[state=active]:text-white bg-[#434D64] border-r border-[#C9A94D]">
                            Property Management
                        </TabsTrigger>
                        <TabsTrigger value="published" className="p-3 h-auto rounded-[10px] w-full md:w-1/2 text-white data-[state=active]:bg-[#135E9A] data-[state=active]:border-[#C9A94D] data-[state=active]:text-white bg-[#434D64]">
