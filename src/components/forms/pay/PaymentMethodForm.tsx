...
const PaymentFormContent: React.FC<{
    ...
    showCheckout?: boolean;
    paymentData?: any;
    isBookingFeePaid?: boolean; // Add this prop
    peaceOfMindEnabled?: boolean; // Add this prop
}> = ({ onSuccess, onCancel, showCheckout = false, paymentData, isBookingFeePaid = false, peaceOfMindEnabled = false }) => {
    const stripe = useStripe();
    const elements = useElements();
    const [createPaymentMethod, { isLoading: isSavingCard }] = useCreatePaymentMethodMutation();
    const [createPayment, { isLoading: isCreatingPayment }] = useCreatePaymentMutation();
    const [confirmPayment, { isLoading: isConfirmingCommission }] = useConfirmPaymentMutation();
    const [createBookingFeePayment, { isLoading: isCreatingBookingFee }] = useCreateBookingFeePaymentMutation();
    const [confirmBookingFeePayment, { isLoading: isConfirmingBookingFee }] = useConfirmBookingFeePaymentMutation();
    ...
    const isLoading = isSavingCard || isCreatingPayment || isConfirmingCommission || isCreatingBookingFee || isConfirmingBookingFee;
    ...
    // Handle checkout with current card - Updated to handle both booking fee and commission payments
    const handleCheckout = async () => {
        if (!stripe || !elements) {
            setCardError("Stripe has not been properly initialized");
            return;
        ...
        if (!paymentData) {
            setCardError("Payment data is missing");
            return;
        }
        ...
        try {
            ...
            // Step 2: Determine which payment flow to use
            if (!isBookingFeePaid) {
                // BOOKING FEE PAYMENT FLOW
                const bookingFeeResponse = await createBookingFeePayment({
                    agreedFee: paymentData.agreedFee,
                    bookingFee: paymentData.bookingFee,
                    extraFee: peaceOfMindEnabled ? paymentData.extraFee || 0 : 0,
                ...
                if (!bookingFeeResponse.success || !bookingFeeResponse.data) {
                    setCardError(bookingFeeResponse.message || "Booking fee payment creation failed");
                    return;
                }
                ...
                if (!paymentIntentId) {
                    setCardError("Missing payment information from server");
                    return;
                }

                // Handle free booking fee
                if (paymentIntentId.startsWith("free_booking_fee")) {
                    toast.success("Booking fee marked as paid!");
                    router.push("/messages");
                ...
                if (confirmResponse.success) {
                    toast.success("Booking fee paid successfully!");
                    router.push("/messages");
                } else {
                    setCardError(confirmResponse.message || "Booking fee payment confirmation failed");
                    return;
                }
            } else {
                // COMMISSION PAYMENT FLOW (booking fee already paid)
                const paymentResponse = await createPayment({
                    agreedFee: paymentData.agreedFee,
                    bookingFee: paymentData.bookingFee,
                    extraFee: 0, // No peace of mind for commission payment
                    totalAmount: paymentData.totalAmount,
                    propertyId: paymentData.propertyId,
                    conversationId: paymentData.conversationId,
                    messageId: paymentData.messageId,
...
interface PaymentMethodFormProps {
    ...
    peaceOfMindEnabled?: boolean;
}

const PaymentMethodForm: React.FC<PaymentMethodFormProps> = ({ onSuccess, onCancel, showCheckout = false, paymentData, isBookingFeePaid = false, peaceOfMindEnabled = false }) => {
    return (
        <Elements stripe={stripePromise}>
            <PaymentFormContent onSuccess={onSuccess} onCancel={onCancel} showCheckout={showCheckout} paymentData={paymentData} isBookingFeePaid={isBookingFeePaid} peaceOfMindEnabled={peaceOfMindEnabled} />
        </Elements>
    );
};
