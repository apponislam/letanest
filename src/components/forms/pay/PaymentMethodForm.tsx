...
const PaymentFormContent: React.FC<{ onSuccess?: () => void; onCancel?: () => void; showCheckout?: boolean }> = ({ onSuccess, onCancel, showCheckout = false }) => {
    const stripe = useStripe();
    const elements = useElements();
    const [createPaymentMethod, { isLoading, error }] = useCreatePaymentMethodMutation();
    ...
    const onSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            // Step 2: Send to our backend using RTK Query
            ...
            // Clear the card element and form
            cardElement.clear();
            setNameOnCard("");
            setEmail("");
            ...
            // Handle RTK Query error or other errors
            const errorMessage = err.data?.message || err.message || "Something went wrong. Please try again.";
            setCardError(errorMessage);
        }
    };
    const handleCheckout = () => {
        console.log("Processing payment with newly added card");
        alert("Processing payment with newly added card!");
    };
    ...
    return (
        <form onSubmit={onSubmit} className="mx-auto space-y-6 max-w-lg">
            ...
            {/* Error Message */}
            {(cardError || error) && <div className="bg-red-900/20 border border-red-500 text-red-200 px-4 py-3 my-2 rounded-lg">{cardError || (error as any)?.data?.message || "An error occurred"}</div>}

            {/* Action Buttons */}
            <div className="flex gap-4 pt-4">
                <button type="submit" disabled={!stripe || loading || !nameOnCard.trim() || !email.trim()} className="flex-1 bg-[#C9A94D] text-white py-3 px-6 rounded-lg hover:bg-[#af8d28] transition-all disabled:opacity-50 disabled:cursor-not-allowed font-medium">
                    {loading ? "Saving Card..." : "Save Card"}
                </button>

                {showCheckout && (
                    <button type="button" onClick={handleCheckout} className="flex-1 bg-[#C9A94D] text-white py-3 px-6 rounded-lg hover:bg-[#af8d28] transition-all font-medium">
                        Check Out
                    </button>
                )}

                {onCancel && (
                    <button type="button" onClick={onCancel} className="flex-1 bg-transparent border border-[#C9A94D] text-[#C9A94D] py-3 px-6 rounded-lg hover:bg-[#C9A94D] hover:text-white transition-all disabled:opacity-50 font-medium" disabled={loading}>
                        Cancel
                    </button>
                )}
            </div>
...
// Main Payment Method Form Component with Stripe Provider
...
const PaymentMethodForm: React.FC<PaymentMethodFormProps> = ({ onSuccess, onCancel, showCheckout = false }) => {
    return (
        <Elements stripe={stripePromise}>
            <PaymentFormContent onSuccess={onSuccess} onCancel={onCancel} showCheckout={showCheckout} />
        </Elements>
    );
};
