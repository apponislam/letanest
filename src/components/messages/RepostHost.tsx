...
interface ReportModalProps {
    isOpen: boolean;
    onClose: () => void;
    reportedUserId: string; // Changed from hostId
    reportedUserName: string; // Changed from hostName
    reportedUserRole: "HOST" | "GUEST"; // Added role
...
const ReportModal: React.FC<ReportModalProps> = ({ isOpen, onClose, reportedUserId, reportedUserName, reportedUserRole, conversationId }) => {
    const [createReport, { isLoading }] = useCreateReportMutation();
    ...
    const onSubmit = async (data: ReportFormData) => {
        try {
            await createReport({
                reportedUserId, // Changed from hostId
            ...
            Swal.fire({
                title: "Success!",
                text: `Report submitted successfully against ${reportedUserRole.toLowerCase()}`,
                icon: "success",
                iconColor: "#C9A94D",
                background: "#2D3546",
                color: "#F5F5F5",
    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleClose}>
            <DialogContent className="bg-[#2D3546] border-[#C9A94D] text-white max-w-md w-full">
                <DialogHeader>
                    <DialogTitle className="text-[#C9A94D] text-xl text-center">Report {reportedUserRole === "HOST" ? "Host" : "Guest"}</DialogTitle>
                    <DialogDescription className="text-gray-300 text-center">
                        Reporting: <span className="text-white font-semibold">{reportedUserName}</span>
                        <span className={`ml-2 px-2 py-1 rounded-full text-xs ${reportedUserRole === "HOST" ? "bg-blue-500" : "bg-green-500"}`}>{reportedUserRole}</span>
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
                    ...
                    <div>
                        ...
                        <Input id="reason" {...register("reason")} placeholder="e.g., Poor communication, Inappropriate behavior, etc." className={`w-full border bg-[#1a202c] text-white placeholder-gray-400 focus:ring-[#C9A94D] ${errors.reason ? "border-red-400" : "border-[#C9A94D]"}`} maxLength={100} />
                        <div className="flex justify-between mt-1">
                            {errors.reason ? <p className="text-red-400 text-sm">{errors.reason.message}</p> : <p className="text-xs text-gray-400">Brief reason for reporting</p>}
                            <p className="text-xs text-gray-400">{reason.length}/100</p>
                        </div>
