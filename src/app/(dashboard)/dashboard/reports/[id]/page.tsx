...
import { useParams } from "next/navigation";
import React from "react";
import { Loader2, MessageSquare, User, Calendar, Home, Phone, Mail, Star, Shield } from "lucide-react";
import Image from "next/image";

// Avatar component for fallback - EXACT SAME AS BEFORE
const Avatar = ({ name, size = 48, className = "", isVerified = false }: { name: string; size?: number; className?: string; isVerified?: boolean }) => {
    const getInitials = (fullName: string) => {
        return fullName
            .split(" ")
...
const MessageBubble = ({ message }: { message: any }) => {
    ...
    const calculateDays = (checkIn: string, checkOut: string) => {
        if (!checkIn || !checkOut) return "0 Night";
        const checkInDate = new Date(checkIn);
        const checkOutDate = new Date(checkOut);
        if (isNaN(checkInDate.getTime()) || isNaN(checkOutDate.getTime())) return "0 Nights";
        const timeDiff = checkOutDate.getTime() - checkInDate.getTime();
        const daysDiff = Math.ceil(timeDiff / (1000 * 60 * 60 * 24)) + 1;
        return `( ${daysDiff} Night${daysDiff !== 1 ? "s" : ""} )`;
    };

    // NEW: Review message (completed)
    if (message.type === "review" && message.reviewed) {
        return (
            <div className="flex justify-start mb-4">
                <div className="bg-[#14213D] p-4 rounded-lg max-w-md border border-[#14213D]">
                    <div className="flex items-center gap-2 mb-3">
                        <div className="w-6 h-6 bg-[#D4BA71] rounded-full flex items-center justify-center">
                            <Star className="w-3 h-3 text-[#14213D]" />
                        </div>
                        <p className="font-bold text-[#D4BA71] text-sm">Review Submitted</p>
                    </div>

                    <div className="text-center">
                        <p className="text-xs text-[#D4BA71] mb-2">Review has been submitted for this conversation</p>
            ...
            <div className="flex justify-start mb-4">
                <div className="bg-[#14213D] p-4 rounded-lg max-w-md border border-[#14213D]">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="font-bold text-[#D4BA71] text-sm">Review Request</p>
                    </div>

                    <div className="text-center">
                        <p className="text-xs text-[#D4BA71] mb-2">Review has been requested for this conversation</p>
                        <p className="text-[10px] text-[#D4BA71]">Requested on: {new Date(message.createdAt).toLocaleDateString()}</p>
                    </div>
                </div>
            </div>
        );
    ...
    // NEW: Make offer (availability request)
            ...
            <div className="flex justify-start mb-4">
                <div className="bg-[#D4BA71] p-4 rounded-lg max-w-md border border-[#C9A94D]">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="font-bold text-[#14213D] text-sm">Availability Request</p>
                    </div>
                    ...
                    <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="font-semibold">Waiting for availability details</span>
                        </div>
                    </div>
    ...
    if (message.type === "system") {
        return (
            <div className="flex justify-center mb-4">
                <div className="bg-gray-100 p-4 rounded-lg max-w-md border border-gray-300">
                    <div className="flex items-center gap-2 mb-2">
                        <Shield className="w-4 h-4 text-gray-600" />
                        <p className="font-bold text-gray-700 text-sm">System Message</p>
                    </div>
                    <div className="text-sm text-gray-600" dangerouslySetInnerHTML={{ __html: message.text }} />
                    <div className="text-xs text-gray-500 mt-2 text-right">{new Date(message.createdAt).toLocaleString()}</div>
                </div>
            </div>
        );
    }

    // Offer message - UPDATED with additional fields
    if (message.type === "offer") {
        const calculateTotal = () => {
            const agreedFee = parseFloat(message.agreedFee) || 0;
            const bookingFee = parseFloat(message.bookingFee) || 0;
        ...
        const totalAmount = message.total || calculateTotal();
        const nights = calculateDays(message.checkInDate, message.checkOutDate);
                ...
                <div className="bg-[#D4BA71] p-4 rounded-lg max-w-md border border-[#C9A94D]">
                    ...
                    <div className="space-y-2 text-sm">
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                            {message.bookingFeePaid && <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded">Booking Fee Paid</span>}
                            {message.hostFeePaid && <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">Host Fee Paid</span>}
                            {message.offerEdited && <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded">Offer Changed</span>}
                        </div>
                    </div>
    ...
    // Request message - UPDATED with additional fields
    if (message.type === "request") {
        const nights = calculateDays(message.checkInDate, message.checkOutDate);
            ...
            <div className="flex justify-start mb-4">
                <div className="bg-blue-100 p-4 rounded-lg max-w-md border border-blue-300">
                    ...
                    <div className="space-y-2 text-sm">
                        ...
                        {message.offerEdited && (
                            <div className="mt-2">
                                <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded">Offer Changed</span>
                            </div>
                        )}
                    </div>
    ...
    // Accepted message - UPDATED with payment status
    if (message.type === "accepted") {
        return (
            <div className="flex justify-start mb-4">
                <div className="bg-green-100 p-4 rounded-lg max-w-md border border-green-300">
        ...
        return (
            <div className="flex justify-start mb-4">
                <div className="bg-red-100 p-4 rounded-lg max-w-md border border-red-300">
                    ...
                    <div className="text-xs text-gray-600 mt-2">
                        <div>Property: {message.propertyId?.propertyNumber || "N/A"}</div>
                        <div>Rejected by: {message.sender?.name}</div>
                        <div>{new Date(message.createdAt).toLocaleString()}</div>
                    </div>
                </div>
    ...
    return (
        <div className="flex justify-start mb-4">
            <div className="bg-[#D4BA71] px-4 py-3 rounded-lg max-w-md break-words text-[#080E1A]">
                <p className="text-sm">{message.text}</p>
                <div className="flex justify-between items-center mt-2 text-xs opacity-70">
                    <span>{message.sender?.name}</span>
                    <span>{new Date(message.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
...
const Page = () => {
    ...
    return (
        <div className="min-h-screen ">
            ...
            <div className="max-w-6xl mx-auto p-6">
                {/* Participants Info - EXACT SAME AS BEFORE */}
                <div className="bg-white rounded-lg shadow-md p-6 mb-6 border border-[#C9A94D]">
                    <h2 className="text-xl font-bold text-[#14213D] mb-4 flex items-center gap-2">
                        <User className="w-5 h-5" />
                        Participants
                ...
                {/* Messages Section - EXACT SAME AS BEFORE */}
                <div className="bg-white rounded-lg shadow-md p-6 border border-[#C9A94D]">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-xl font-bold text-[#14213D] flex items-center gap-2">
                            <MessageSquare className="w-5 h-5" />
